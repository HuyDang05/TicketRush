...
function eventMatchesSearch(event, query) {
  ...
  return event.title?.toLowerCase().includes(needle) || event.venue?.toLowerCase().includes(needle);
}
function getCachedSearchSuggestions(query) {
  if (searchSuggestionCache.has(query)) {
    return searchSuggestionCache.get(query);
  }
  const cachedPrefix = [...searchSuggestionCache.keys()].filter(key => query.startsWith(key)).sort((a, b) => b.length - a.length)[0];
  if (!cachedPrefix) return null;
  return searchSuggestionCache.get(cachedPrefix).filter(event => eventMatchesSearch(event, query)).slice(0, SEARCH_SUGGESTION_LIMIT);
}
export default function Header() {
  ...
  useEffect(() => {
    ...
    const cachedSuggestions = getCachedSearchSuggestions(normalizedSearch);
    if (cachedSuggestions) {
      setSuggestions(cachedSuggestions);
      if (searchSuggestionCache.has(normalizedSearch)) {
        return undefined;
      }
    }
    ...
    const timeoutId = window.setTimeout(() => {
      eventService.getEventSuggestions({
        ...
        searchSuggestionCache.set(normalizedSearch, events);
  ...
  const langIconVi = <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const langIconEn = <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return <>
      <header className="header">
        ...
        <div className="header__search" ref={searchRef}>
          <input type="text" placeholder={t("Tìm kiếm sự kiện, nghệ sĩ...")} value={search} maxLength={MAX_SEARCH_LENGTH} onChange={handleSearchChange} onFocus={() => setShowDropdown(true)} onKeyDown={handleSearch} className="header__search-input" />
          <button onClick={() => {
          if (normalizedSearch) {
            setShowDropdown(false);
            navigateToEventsSearch(normalizedSearch);
          ...
          {showDropdown && suggestions.length > 0 && <div className="search-dropdown">
              {suggestions.map(event => <button key={event.id} className="search-dropdown__item" onMouseDown={() => handleSuggestionClick(event)}>
        ...
        <div className="header__actions">
          <div className="header__lang-menu" ref={langMenuRef}>
            <button type="button" className="header__theme-toggle header__lang-btn" onClick={() => setShowLangDropdown(!showLangDropdown)} title={t("Ngôn ngữ")}>
              {lang === 'vi' ? langIconVi : langIconEn}
            </button>
            {showLangDropdown && <div className="header__dropdown">
                <button className={cx("header__dropdown-item", css({
              display: 'flex',
              alignItems: 'center',
              gap: 12,
              fontWeight: lang === 'vi' ? 600 : 400
            }, "Header"))} onClick={() => handleLangChange('vi')}>
            ...
            }, "Header"))} onClick={() => handleLangChange('en')}>
                  {langIconEn} {t("English")}
                </button>
              </div>}
          </div>

          <button type="button" className="header__theme-toggle" onClick={toggleTheme} aria-label={theme === 'dark' ? 'Chuyen sang theme sang' : 'Chuyen sang theme toi'} title={theme === 'dark' ? 'Theme sang' : 'Theme toi'}>
          ...
          {user && <button onClick={() => navigate('/cart')} className={cx("header__cart-btn", css({
          ...
          width: 40,
          height: 40
        }, "Header"))} title={lang === 'en' ? 'Cart' : 'Giỏ hàng'}>
              <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              {cartCount > 0 && <span className={css({
            position: 'absolute',
            top: 0,
            right: 0,
            background: '#ef4444',
          ...
          {!user ? <>
              ...
              <button type="button" className="header__account-btn" onClick={() => setOpenUserMenu(prev => !prev)}>
                {avatarUrl ? <img src={avatarUrl} alt="avatar" className="header__avatar-img" onError={e => {
setNodeCss(e.currentTarget, { display: 'none' }, 'display');
setNodeCss(e.currentTarget.nextSibling, { display: 'flex' }, 'display');
              ...
              {openUserMenu && <div className="header__dropdown">
                  {user.role === 'CUSTOMER' && <button type="button" className="header__dropdown-item" onClick={() => {
              setOpenUserMenu(false);
              navigate('/my-tickets');
            }}>
                  ...
                  <button type="button" className="header__dropdown-item" onClick={() => {
              setOpenUserMenu(false);
              navigate('/account');
            }}>
                  ...
                  {user.role === 'ADMIN' && <button type="button" className="header__dropdown-item" onClick={() => {
              setOpenUserMenu(false);
              navigate('/admin');
            }}>
                      Admin Dashboard
