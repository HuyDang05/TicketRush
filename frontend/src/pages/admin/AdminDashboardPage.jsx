...
function drawSparkline(canvas) {
  ...
  ctx.strokeStyle = 'var(--success)'; ctx.lineWidth = 1.5; ctx.stroke();
}
...
function drawRevenue(canvas, offset = 0) {
  ...
  for (let i = 0; i <= gridLines; i++) {
    ...
    ctx.strokeStyle = 'rgba(120,120,120,0.12)'; ctx.lineWidth = 1; ctx.stroke();
  ...
  ctx.strokeStyle = 'var(--accent)'; ctx.lineWidth = 2; ctx.lineJoin = 'round'; ctx.stroke();
...
function DonutChart({ sold, held, total }) {
  ...
  const segs = [
    { val: sold,  color: 'var(--accent)' },
    { val: held,  color: 'var(--muted)' },
    { val: avail, color: 'var(--border)' },
  ];
  let angle = -Math.PI / 2;
  const paths = [];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'absolute', textAlign: 'center', pointerEvents: 'none' }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>Lấp đầy</div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', justifyContent: 'center' }}>
        {[['var(--accent)','Đã bán'],['var(--muted)','Đang giữ'],['var(--border)','Còn trống']].map(([color,label]) => (
      ...
      {entries.filter(e => e.val > 0 || e.label !== '?').map(({ label, val }) => {
        ...
        return (
          <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, flex: 1 }}>
            <span style={{ fontSize: 10, color: 'var(--muted)', fontWeight: 600 }}>{val}</span>
            <div style={{ width: '100%', height: `${pct}%`, borderRadius: '4px 4px 0 0', background: 'var(--accent)', opacity: 0.5 + pct / 200 }} />
            <span style={{ fontSize: 10, color: 'var(--muted)', textAlign: 'center', whiteSpace: 'nowrap' }}>{label}</span>
          </div>
        );
      })}
    </div>
...
function zoneStatus(soldPct) {
  if (soldPct >= 90) return { label: 'Sắp hết', color: 'var(--success)', bg: 'rgba(74,222,128,.1)', border: 'rgba(74,222,128,.25)' };
  if (soldPct >= 40) return { label: 'Còn vé',  color: 'var(--accent)', bg: 'rgba(255,107,53,.1)', border: 'rgba(255,107,53,.25)' };
  return { label: 'Còn nhiều', color: 'var(--muted)', bg: 'var(--bg)', border: 'var(--border)' };
}
...
export default function AdminDashboardPage() {
  ...
  const genderBars = [
    ...
    { label: 'Khác', count: genderMap.OTHER  || 0, color: 'var(--muted)' },
  ];
  ...
  return (
    <AdminLayout>
      <style>{`
        ...
        .dash-tr:hover td{background:rgba(120,120,120,0.08)}
      `}</style>

      {/* TOP BAR */}
      <div style={{ background: 'var(--nav)', borderBottom: '1px solid var(--border)', padding: '16px 28px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <select
              ...
              style={{
                ...
                padding: '7px 28px 7px 12px', color: 'var(--text)', fontFamily: 'inherit',
            ...
            <button
              ...
              style={{
                ...
                color: 'var(--muted)', fontFamily: 'inherit', fontSize: 12, fontWeight: 600,
                cursor: 'pointer', transition: 'all .2s',
              }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(120,120,120,0.3)'; e.currentTarget.style.color = 'var(--text)'; }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0, animation: 'live-pulse 1.5s ease-in-out infinite' }} />
          <span style={{ fontSize: 12, color: 'var(--muted)' }}>
            Đang cập nhật theo thời gian thực · Cập nhật lần cuối: <span>{lastUpdate}</span>
          </span>
          {loading && <span style={{ fontSize: 11, color: 'var(--accent)', marginLeft: 4 }}>Đang tải...</span>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 10 }}>Tổng ghế</div>
            <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1, marginBottom: 8 }}>{totalSeats}</div>
            <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 8, overflow: 'hidden' }}><div style={{ height: '100%', width: '100%', background: 'var(--border)', borderRadius: 2 }} /></div>
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>Sức chứa tối đa</div>
          </div>
          {/* Sold */}
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 10 }}>Đã bán</div>
            <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1, marginBottom: 8, color: 'var(--success)' }}>{soldSeats}</div>
            <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 8, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${totalSeats > 0 ? (soldSeats / totalSeats * 100) : 0}%`, background: 'var(--success)', borderRadius: 2, transition: 'width .8s ease' }} />
            </div>
          ...
          {/* Locked */}
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 10 }}>Đang giữ</div>
            <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1, marginBottom: 8, color: 'var(--accent)' }}>{lockedSeats}</div>
            <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 8, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${totalSeats > 0 ? (lockedSeats / totalSeats * 100) : 0}%`, background: 'var(--accent)', borderRadius: 2, transition: 'width .8s ease' }} />
            </div>
            <div style={{ fontSize: 11, color: 'var(--accent)' }}>Sẽ nhả trong vài phút</div>
          </div>
          {/* Revenue */}
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 10 }}>Doanh thu</div>
            <div style={{ fontSize: 22, fontWeight: 800, lineHeight: 1, marginBottom: 8, color: 'var(--accent)' }}>{formatRevenue(revenue)}</div>
            <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 8, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${occupancyRate}%`, background: 'var(--accent)', borderRadius: 2, transition: 'width .8s ease' }} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--success)' }}>
              <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M18 15l-6-6-6 6"/></svg>
              +12% so với hôm qua
            ...
            <thead>
              <tr style={{ background: 'var(--bg)', borderRadius: 8 }}>
                {['Khu vực','Tổng ghế','Đã bán','Tỷ lệ','Doanh thu','Trạng thái'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'var(--muted)', letterSpacing: .8, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(dash?.zones || []).map((zone, idx) => {
                ...
                return (
                  <tr key={zone.zoneId} className="dash-tr" style={{ borderTop: '1px solid rgba(120,120,120,0.1)' }}>
                    ...
                    <td style={{ padding: '12px 14px', fontSize: 12, verticalAlign: 'middle', color: 'var(--accent)', fontWeight: 700 }}>
              ...
              {(!dash?.zones || dash.zones.length === 0) && (
                <tr><td colSpan={6} style={{ padding: '20px 14px', textAlign: 'center', color: 'var(--muted)', fontSize: 12 }}>Chọn sự kiện để xem dữ liệu</td></tr>
              )}
            </tbody>
          </table>
        </div>
