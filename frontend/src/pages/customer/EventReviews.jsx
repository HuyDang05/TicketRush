...
export default function EventReviews({ eventId }) {
  ...
  useEffect(() => {
    if (!eventId) return;
    setIsLoading(true);
    eventService.getEventReviews(eventId)
      .then((res) => {
  ...
  const renderStars = (rating, isInteractive = false) => {
    return Array.from({ length: 5 }).map((_, i) => {
      ...
      return (
        <svg 
          key={i} 
          className={`er-star ${isFilled ? 'er-star--filled' : ''} ${isInteractive ? 'er-star--interactive' : ''}`} 
          width="18" height="18" viewBox="0 0 24 24" fill="currentColor"
          onMouseEnter={isInteractive ? () => setHoverRating(starValue) : undefined}
          onMouseLeave={isInteractive ? () => setHoverRating(0) : undefined}
          onClick={isInteractive ? () => setFormRating(starValue) : undefined}
        >
  ...
  return (
    <div className="ed-card er-container">
      <div className="er-header">
        <h2 className="ed-card__title er-title">Đánh giá & Bình luận</h2>
        <button 
          className={`er-write-btn ${isWriting ? 'er-write-btn--active' : ''}`} 
          onClick={handleStartWriting}
        >
          {isWriting ? 'Đóng form' : 'Viết đánh giá'}
        </button>
      </div>
      ...
      <div className="er-stats">
        <div className="er-stats__main">
          ...
          <div className="er-stats__details">
            ...
            <div className="er-stats__total">{totalReviews} bài đánh giá</div>
          </div>
        </div>
      </div>
      ...
      {/* Write Form */}
      {isWriting && (
        <div className="er-form-wrapper">
          <div className="er-form">
            <div className="er-form__rating">
              <span className="er-form__label">Đánh giá của bạn:</span>
              <div className="er-form__stars">{renderStars(0, true)}</div>
            </div>
            
            <textarea 
              className="er-form__textarea" 
            ...
            {imagePreview && (
              <div className="er-form__preview">
                <img src={imagePreview} alt="Ảnh preview" />
      ...
      <div className="er-list">
        {isLoading && <div className="er-empty">Đang tải đánh giá...</div>}
        {!isLoading && reviews.length === 0 && (
          <div className="er-empty">Chưa có đánh giá nào</div>
        )}
        {!isLoading && reviews.map((review, index) => (
          <div key={review.id} className="er-card" style={{ animationDelay: `${index * 0.1}s` }}>
            <div className="er-card__header">
              <img src={review.user?.avatar || 'https://i.pravatar.cc/150?u=guest'} alt={review.user?.name || 'Ẩn danh'} className="er-card__avatar" />
              <div className="er-card__user-info">
                <div className="er-card__name">{review.user?.name || 'Ẩn danh'}</div>
            ...
            {review.imageUrl && (
              <div className="er-card__media">
                <img src={review.imageUrl} alt="Trải nghiệm sự kiện" loading="lazy" />
              </div>
            )}
          </div>
        ))}
